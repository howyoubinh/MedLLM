...
def format_input_string(preamble:str,instruction:str,question:str,options:str):
...
def prepare_data(data_list: Sequence[dict],augmentation:str="") -> Sequence[dict]:
    for _idx in tqdm(range(len(data_list))):
        question = data_list[_idx]['Question']
        options = data_list[_idx]['Options']
        data_list[_idx]['Augmentation'] = augmentation
        if augmentation == "":
            formatted_string = format_input_string(preamble,
                                                   instruction,
                                                   question,
                                                   options,)
            data_list[_idx]['pmc_input'] = formatted_string
        else:
            raise ValueError("Augmentation Not yet implemented")
    return data_list
...
def parse_args():
    ...
    parser.add_argument('--no-entities', action='store_true',help='Disable entity extraction') # add this argument to extract entities
...
if __name__ == '__main__':
    ...
    data_list = prepare_data(data_list,augmentation=args.augmentation)
    for entry in data_list[0:1]:
    ...
    extractor = None
    if args.no_entities:
        print(f"\033[No Entity Extractor\033[0m")
        extractor = None
    else:
